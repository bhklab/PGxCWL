getUHN <-
  function (#gene=TRUE,
    ...
    sensAZD <- sensitivity$info[grep("AZD6244_rep_1", rownames(sensitivity$info)),]
    sensLapatinib <- sensitivity$info[grep("lapatinib_rep_2", rownames(sensitivity$info)),]
    sensPaclitaxel <- sensitivity$info[grep("paclitaxel_rep_2", rownames(sensitivity$info)),]
    sensErlotinib1 <- sensitivity$info[grep("erlotinib_rep_1", rownames(sensitivity$info)),]
    sensErlotinib1 <- sensErlotinib1[-grep("HCC1187_erlotinib_rep_1", rownames(sensErlotinib1)),]
    sensfinal <- rbind(sensAZD, sensLapatinib,sensPaclitaxel,sensErlotinib1)
    sensitivity$info <- sensfinal
    
    sensitivity$profiles <- sensitivity$profiles[rownames(sensitivity$info),]
    sensitivity$raw <- sensitivity$raw[rownames(sensitivity$profiles),,"Dose"]
    ...
    druginfo <- druginfo[-c(2,6,7,8),,drop=F]
    rownames(druginfo) <- druginfo$drugid
    ...
    UHNBreast2017 <- PharmacoSet(name="UHNBreast", 
                                molecularProfiles = z,
                                cell= cellineinfo,
                                drug=druginfo,
    ...
    save(UHNBreast2017,file="UHN_2017.RData")
    
   
   return(UHNBreast2017)
