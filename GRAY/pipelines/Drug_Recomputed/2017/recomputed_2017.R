...
getGRAYrawData <-
  function(result.type=c("array", "list")){
    ...
    raw = commandArgs(trailingOnly=TRUE)[1]
    ...
    conc = commandArgs(trailingOnly=TRUE)[2]
...
GRraw = commandArgs(trailingOnly=TRUE)[3]
gray.GRvalues <- read.csv(file= GRraw, header=T, stringsAsFactors=FALSE, sep="\t")

crosscell = commandArgs(trailingOnly=TRUE)[4]
crossdrug= commandArgs(trailingOnly=TRUE)[5]
...
save(raw.sensitivity, sensitivity.info, tt, con_tested, cells.cross.reference, drugs.cross.reference, gray.GRvalues, gray2013sens, file="drug_norm_post2017.RData")

.calculateSensitivitiesStar <-
  function (pSets = list(), exps=NULL, cap=NA, na.rm=TRUE, area.type=c("Fitted","Actual"), nthread=5) {
